...
public class KhachHangDAO extends DAO {
    ...
    /**
     * Xử lý chính sách giá dựa theo đối tượng và khởi tạo Hóa Đơn tạm (Đã khử
     * sạch lỗi biên dịch)
     */
    public HoaDon xuLyChinhSachGiaVaTaoDonTam(String hoTen, String soCCCD, String soDT, String email, String loaiDoiTuongStr, LichTrinh lt, String toa, String maGhe, double giaGoc) throws Exception {
        // 1. Kiểm tra mã khách hàng từ DB dựa theo CCCD, ánh xạ đúng tên cột trong cơ sở dữ liệu thật
        ...
        try (PreparedStatement ps = con.prepareStatement(sqlKH)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    // ĐỒNG BỘ: Sử dụng đúng tên cột DB và constructor chuẩn của nhóm
                    kh = new KhachHang(
                            rs.getString("maKH"),
                            rs.getString("hoTen"),
                            rs.getString("soCCCD"),
        ...
        if (kh == null) {
            ...
            try (PreparedStatement ps = con.prepareStatement(sqlInsert)) {
                // ĐỒNG BỘ: Gọi chuẩn tên phương thức có trong thực thể KhachHang.java của nhóm
        ...
        // 3. Đọc phần trăm giảm giá từ bảng ChinhSachGia trong DB thật
        ...
        // 5. Khởi tạo đối tượng HoaDon chuẩn hệ thống (Đồng bộ chuẩn tên hàm với module Thống kê)
        HoaDon bill = new HoaDon();
        bill.setMaHoaDon("HD-" + UUID.randomUUID().toString().substring(0, 4).toUpperCase());
        bill.setLoaiHoaDon("Mua ve"); // ĐỒNG BỘ: Đổi sang setLoaiHoaDon theo đúng thuộc tính bạn ông thiết kế
